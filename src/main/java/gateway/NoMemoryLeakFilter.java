...
public class NoMemoryLeakFilter implements GatewayFilter {

	private static final Logger logger = LoggerFactory.getLogger(NoMemoryLeakFilter.class);
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		logger.info("[NoMemoryLeakFilter] filter");
		...
		logger.warn("[NoMemoryLeakFilter] directBufferPreferred : {}", directBufferPreferred);
		logger.warn("[NoMemoryLeakFilter] usedDirectMemory : {}", usedDirectMemory);
		logger.warn("[NoMemoryLeakFilter] maxDirectMemory : {}", maxDirectMemory);
...
//					logger.info("[NoMemoryLeakFilter] content2 : {}", content);
					...
					ServerHttpRequest requestDecorator = new ServerHttpRequestDecorator(exchange.getRequest()) {
						...
						public Flux<DataBuffer> getBody() {
							logger.info("[NoMemoryLeakFilter] getBody call");
