...
public class MultiGetBodyFilter1 implements GatewayFilter {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		logger.info("[MultiGetBodyFilter1] filter");
		...
		logger.warn("[MultiGetBodyFilter1] directBufferPreferred : {}", directBufferPreferred);
		logger.warn("[MultiGetBodyFilter1] usedDirectMemory : {}", usedDirectMemory);
		logger.warn("[MultiGetBodyFilter1] maxDirectMemory : {}", maxDirectMemory);
		...
		return DataBufferUtils.join(request.getBody())
				.flatMap(dataBuffer -> {
					...
					logger.info("[MultiGetBodyFilter1] requestBodyMap : {}", requestBodyMap);

					ServerHttpRequest requestDecorator = new ServerHttpRequestDecorator(exchange.getRequest()) {
						@Override
						public Flux<DataBuffer> getBody() {
							logger.info("[MultiGetBodyFilter1] getBody call");
