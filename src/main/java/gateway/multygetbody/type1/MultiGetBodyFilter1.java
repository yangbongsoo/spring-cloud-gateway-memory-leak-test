...
public class MultiGetBodyFilter1 implements GatewayFilter {

	private static final Logger logger = LoggerFactory.getLogger(MultiGetBodyFilter1.class);
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		logger.info("[MultyGetBodyFilter1] filter");
		...
		logger.warn("[MultyGetBodyFilter1] directBufferPreferred : {}", directBufferPreferred);
		logger.warn("[MultyGetBodyFilter1] usedDirectMemory : {}", usedDirectMemory);
		logger.warn("[MultyGetBodyFilter1] maxDirectMemory : {}", maxDirectMemory);
		...
		return DataBufferUtils.join(request.getBody())
				.flatMap(dataBuffer -> {
					...
					logger.info("[MultyGetBodyFilter1] requestBodyMap : {}", requestBodyMap);

					// NettyRoutingFilter 까지 보내지 않고 해당 필터에서 끝내면 따로 release를 해줘야함. 안하면 메모리 누수 발생
